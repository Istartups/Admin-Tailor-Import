...
import { Home, Grid3X3, Star, Crown, Settings } from "lucide-react";
...
const navItems = [
  ...
  { path: "/pre-unlock",    label: "Unlock",    icon: Crown },
...
export default function BottomNav() {
  ...
  const isActive = (path: string) => {
    ...
    if (path === "/all-tools") {
      return location === "/all-tools" && !search;
    }
    ...
    >
      <div className="flex items-center justify-around h-16 px-1 max-w-lg mx-auto">
        {navItems.map(({ path, label, icon: Icon }) => {
          ...
          const displayLabel = label === "Unlock" ? (isPremium ? "Premium" : "Unlock") : label;
          return (
            <button
              key={path}
            ...
            >
              ...
              />
              <span className="text-[9px] font-medium relative" style={{ color: active ? "hsl(43,82%,55%)" : "hsl(218,20%,50%)" }}>
                {displayLabel}
              </span>
            </button>
          );
